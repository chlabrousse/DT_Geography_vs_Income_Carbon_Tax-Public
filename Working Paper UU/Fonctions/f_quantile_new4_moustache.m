function m = f_quantile_new4_moustache(p, n_quantile, density, s, varargin)
...
for n = 1:length(nom)
    ...
    for iq = 1:n_quantile   
        ...
        % Dispersion of the variable inside quintile
        MM = [d1/sum(d1), M1] ;
        M2  = sortrows(MM,2);         
        for i_quart = 1:length(quart_vec)
            qq = quart_vec(i_quart) ;
            quart = find(cumsum(M2(:,1))< qq, 1, 'last') ;
            m.quintile.disp.(var)(i_quart,iq) = M2(quart,2) ;
        end
    end
    % TYPE
    ...
    for ik = 1:p.nk   
        ...
        % Dispersion of the variable inside type
        MM = [d1/sum(d1), M1] ;
        M2  = sortrows(MM,2);         
        for i_quart = 1:length(quart_vec)
            qq = quart_vec(i_quart) ;
            quart = find(cumsum(M2(:,1))< qq, 1, 'last') ;
            m.type.disp.(var)(i_quart,ik) = M2(quart,2) ;
        end
    end
    % TYPE * QUINTILE
    for ik = 1:p.nk   
