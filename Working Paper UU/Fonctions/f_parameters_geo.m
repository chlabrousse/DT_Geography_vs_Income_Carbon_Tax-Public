...
p.Lambda_E = 0.095443878673983 ; % share energy
...
ebar_grid = [0.018193653679243, 0.014333425305482, 0.012951232563184, 0.005897477462373,  0] ; % energy requirement by city
p.ebar     = kron(ebar_grid(:),ones(p.nb*p.nz,1)) ;  % grid energy requirement by city
gamma_grid = [0.828972352143578, 0.806994170421513, 0.801870069809606, 0.796240829383335, 0.726027923676870] ; % share of F by city
...
% Housing supply
H_SS = [0.431948494429433, 0.460306565702141, 0.288870342197461, 0.198089183703169, 0.323731186433233]' ;
p_SS = [0.034743184978300, 0.040124407999651, 0.056800276112379, 0.061681191893868, 0.058909710947366]' ;
...
% Migration cost
p.cost_matrix(1,:) = [0       0.18    0.35     0.5     1.9]; 
p.cost_matrix(2,:) = [0.19    0       0.32     0.45    1.9]; 
p.cost_matrix(3,:) = [0.12    0.06    0        0.3     1.8]; 
p.cost_matrix(4,:) = [0.06    0.01    0.035    0       1.6]; 
p.cost_matrix(5,:) = [0.0     0.0     0.0      0.0     0]; 
...
tau_h_final = 1.249797909964253 ;
tau_f_final = 0.665583969629451 ;
