function QQ = fig_W_cross_double(p, ss, tr1, tr2)
...
%WE10 = f_calc_CEV(p, ss, V_SS, V_trans10 ) ;
%WE01 = f_calc_CEV(p, ss, V_SS, V_trans01 ) ;
...
% scale = 1;
subplot(121) ; plot(QQ.cross.WE10,'linewidth',4) ; 
ax2 = gca; title('a. $\tau^h$ only', 'interpreter','latex')  ;
axes  
subplot(122) ; plot(QQ.cross.WE01,'linewidth',4) ; 
ax2 = gca; title('b. $\tau^f$ only', 'interpreter','latex')  ;
% ylim([-1 -.4]); yticks(-1:.2:0) ;
