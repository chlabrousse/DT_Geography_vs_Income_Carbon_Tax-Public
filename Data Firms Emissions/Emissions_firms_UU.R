...
## NOTE: TÉLÉCHARGER LE FICHIER DE DIFFUSION DES CODES COMMUNES PAR TYPE D'UNITÉ URBAINE
unite_urbaine <- read_excel("UU2010_au_01-01-2020.xlsx", skip = 5)
communes_data <- read_excel("UU2010_au_01-01-2020.xlsx", sheet = "Composition_communale", skip = 5)
setDT(communes_data)
setDT(unite_urbaine)

communes_data[unite_urbaine,TUU2017 := i.TUU2017,on = .(UU2010)]
communes_data[is.na(TUU2017), TUU2017 := 0]
communes_data[TUU2017 == 0, all(STATUT_2017 == "R")] # TRUE if all R = 0


# Pad the code_commune_INSEE variable with leading zeros to ensure a width of 5
communes_data[, code_commune_Insee := CODGEO]

# Remove rows with missing values
communes_data <- na.omit(communes_data)

communes_data[, city_size_type :=
                fifelse(TUU2017 == 8, "Paris",
                        fifelse(TUU2017 == 0, "Rural",
                                fifelse(TUU2017 %in% c(1, 2), "Small cities",
                                        fifelse(TUU2017 %in% c(3, 4), "Medium cities",
                                                fifelse(TUU2017 %in% c(5, 6, 7), "Large cities",
                                                        NA_character_)))))]
...
Emissions_per_sector <- read_csv("XXXXXX/data_emissions_IPP_CN.csv", 
...
N_tot <- nrow(data_uniq)

# Start table: one row per sector (a88)
result <- data_uniq[, .(n_sector = .N), by = a88]

# Share of sector in total employment
result[, share_total := n_sector / N_tot*100 ]

# ---------- CITY SIZE SHARES ----------

# Counts by sector and city size
city_counts <- data_uniq[, .(n = .N), by = .(a88, city_size_type)]

# Merge with sector totals
city_counts <- merge(city_counts, result[, .(a88, n_sector)], by = "a88")

# Compute shares within sector
city_counts[, share := n / n_sector]

# Cast wide (no dots, no chaining)
city_wide <- dcast(
  city_counts,
  a88 ~ city_size_type,
  value.var = "share",
...
# Counts by sector and quintile
quint_counts <- data_uniq[, .(n = .N), by = .(a88, quintile)]

# Merge with sector totals
quint_counts <- merge(quint_counts, result[, .(a88, n_sector)], by = "a88")

# Compute shares within sector
quint_counts[, share := n / n_sector]
...
final_table <- merge(result[, .(a88, share_total)], city_wide, by = "a88", all = TRUE)
final_table <- merge(final_table, quint_wide, by = "a88", all = TRUE)
...
final_table_save = final_table

setDT(final_table)

# ----------------------------
# 1) CITY SHARES IN TOTAL POPULATION
# ----------------------------

city_shares <- data_uniq[, .N, by = city_size_type]
city_shares[, share_city_total := N / sum(N)]

# Named vector
city_share_vec <- city_shares$share_city_total
names(city_share_vec) <- city_shares$city_size_type

# ----------------------------
# 2) NORMALISE CITY COLUMNS
# ----------------------------

final_table[, Rural         := (Rural / city_share_vec["Rural"]) * share_total]
final_table[, `Small cities` := (`Small cities` / city_share_vec["Small cities"]) * share_total]
final_table[, `Medium cities`:= (`Medium cities` / city_share_vec["Medium cities"]) * share_total]
final_table[, `Large cities` := (`Large cities` / city_share_vec["Large cities"]) * share_total]
final_table[, Paris         := (Paris / city_share_vec["Paris"]) * share_total]

# ----------------------------
# 3) NORMALISE QUINTILES (each = 20%)
# ----------------------------

final_table[, Q1 := (Q1 / 0.2) * share_total]
final_table[, Q2 := (Q2 / 0.2) * share_total]
final_table[, Q3 := (Q3 / 0.2) * share_total]
final_table[, Q4 := (Q4 / 0.2) * share_total]
final_table[, Q5 := (Q5 / 0.2) * share_total]
